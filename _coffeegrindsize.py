...
class App(Frame):
    ...
    def __init__(self, master=None):
        ...
        # Remember the root object for the full user interface so that the methods of App can refer to it
        self.master = root
        ...
        self.master.bind_all("<Command-e>", lambda event: toggle_expert_mode(self))
    ...
    def create_canvas_frame(self):
        ...
        self.image_canvas = Canvas(
            self.container_options, width=self.canvas_width, height=self.canvas_height,
        )
    ...
    def create_top_toolbar(self):
        ...
        region_button.pack(side=LEFT)

        # Button to apply image threshold
        threshold_image_button = Button(
            toolbar, text="Threshold Image", command=lambda: self.threshold_image(None)
        )
        threshold_image_button.pack(side=LEFT)
        ...
        erase_button = Button(
            toolbar, text="Erase Clusters", command=lambda: self.erase_clusters(None)
        )
        ...
        # Button to open blog
        blog_button = Button(
            toolbar, text="Read Coffee Ad Astra Blog", command=blog_goto
        )
        blog_button.pack(side=RIGHT)

        # Downsample button
        downsample_button = Button(
            toolbar2, text="Reduce Image Quality", command=self.downsample_image
        )
        downsample_button.pack(side=LEFT)

        # Button to load data from disk
        load_data_button = Button(
            toolbar2, text="Load Data", command=lambda: self.load_data(None)
        )
        ...
        load_comparison_data_button.pack(side=LEFT)

        # Button to flush comparison data
        flush_comparison_data_button = Button(
            toolbar2, text="Flush Comparison Data", command=self.flush_comparison_data
        )
        flush_comparison_data_button.pack(side=LEFT)

        # Button to output data to the disk
        save_button = Button(
            toolbar2, text="Save Data", command=lambda: self.save_data(None)
        )
        save_button.pack(side=LEFT)

        # Button to save histogram to disk
        savehist_button = Button(
            toolbar2, text="Save View", command=lambda: self.save_histogram(None)
        )
        savehist_button.pack(side=LEFT)

        # Quit button
        quit_button = Button(toolbar2, text="Quit", command=quit_gui,)
        quit_button.pack(side=RIGHT)

        # Reset button
        reset_button = Button(toolbar2, text="Reboot", command=reset_gui,)
        reset_button.pack(side=RIGHT)

        # Help button
        help_button = Button(toolbar2, text="Help", command=launch_help,)
        ...
        subMenu.add_command(
            label="Turn Expert Mode On/Off...", command=toggle_expert_mode(self)
        )
        subMenu.add_separator()
        ...
        subMenu.add_command(label="Quit", command=quit_gui)
    ...
    def create_bottom_statusbar(self):
        ...
        status = Label(
            self.master, textvariable=self.status_var, anchor=W, relief=SUNKEN,
        )
    ...
    def create_analysis_widgets(self):
        ...
        # CreateLabelSeparator(self,)

        # All options related to image scale
        CreateTitleLabel(self, "Physical Scale of the Image:")
        ...
        self.reference_object = CreateDropdown(
            self,
            "Reference Object:",
        ...
        self.pixel_scale_var = CreateEntryWithLabels(
            self,
            def_pixel_scale,
            "Pixel Scale:",
            "pix/mm",
        ...
        CreateTitleLabel(self, "Threshold Step:", advanced=True)

        # Value of fractional threshold in units of flux in the blue channel of the image
        self.threshold_var = CreateEntryWithLabels(
            self, def_threshold, "Threshold:", "%", advanced=True, clear_on_click=True,
        )

        # CreateLabelSeparator(self,)

        # All options related to particle detection
        CreateTitleLabel(self, "Particle Detection Step:", advanced=True)

        # Maximum cluster diameter that should be considered a valid coffee particle
        self.max_cluster_axis_var = CreateEntryWithLabels(
            self,
        ...
        self.min_surface_var = CreateEntryWithLabels(
            self,
            def_min_surface,
        ...
        self.min_roundness_var = CreateEntryWithLabels(
            self,
            def_min_roundness,
            "Minimum Roundness:",
            "",
        ...
        if self.display_advanced_options is True:
            self.reference_threshold_var = CreateEntryWithLabels(
                self, def_reference_threshold, "Ref. Threshold:", "", advanced=True
            )
        else:
        ...
        if self.display_advanced_options is True:
            self.maxcost_var = CreateEntryWithLabels(
                self, def_maxcost, "Max. Cost:", "", advanced=True
            )
        else:
        ...
        CreateLabelSeparator(self, simpleonly=True)

        # All options related to particle detection
        CreateTitleLabel(self, "Create Histogram Step:")

        self.histogram_type = CreateDropdown(
            self,
            "Histogram Options:",
            hist_choices,
            self.change_histogram_type,
            default_choice_index=default_histogram_choice,
        )

        self.legend_type = CreateDropdown(
            self, "Label Position:", legend_choices, self.change_histogram_type
        )

        # Label for the data
        self.data_label_var = CreateEntryWithLabels(
            self,
            "Current Data",
            "Data Label:",
            "",
        ...
        self.xmin_var, self.xmin_var_id = CreateEntryWithLabels(
            self,
            def_min_x_axis,
            "Min. X Axis:",
            "",
        ...
        self.xmax_var, self.xmax_var_id = CreateEntryWithLabels(
            self,
            def_max_x_axis,
            "Max. X Axis:",
            "",
        ...
        # X axis range for the histogram figure
        self.nbins_var, self.nbins_var_id = CreateEntryWithLabels(
            self,
            10,
            "Num. bins:",
            "",
        ...
        CreateTitleLabel(self, "Output Options:", advanced=True)
        ...
        self.output_dir_var, self.output_dir_label_id = CreateEntryWithLabels(
            self,
            self.output_dir,
            "",
            "",
        ...
        self.session_name_var = CreateEntryWithLabels(
            self,
            def_session_name,
            "Base of File Names:",
            "",
        ...
        CreateTitleLabel(self, "Display Options:")
        ...
        self.display_type = CreateDropdown(
            self, "Display Type:", choices, self.change_display_type
        )
    ...
    def create_stats_frame(self):
        ...
        self.frame_stats = Frame(self.container_options)
        ...
        title_label.grid(row=0, sticky=W, columnspan=12)
        ...
        diam_average_label = Label(
            self.frame_stats, text="Average Diameter:", font="Helvetica 14 bold",
        )
        ...
        unit_label = Label(self.frame_stats, text="(mm)",)
        ...
        diam_stddev_label = Label(
            self.frame_stats, text="Scatter in Diameter:", font="Helvetica 14 bold",
        )
        ...
        separator_label = Label(self.frame_stats, text="", width=stats_colsep_width,)
        ...
        surf_average_label = Label(
            self.frame_stats, text="Average Surface:", font="Helvetica 14 bold",
        )
        ...
        surf_stddev_label = Label(
            self.frame_stats, text="Scatter in Surface:", font="Helvetica 14 bold",
        )
        ...
        # ey_average_label = Label(self.frame_stats, text="Average EY:",, font='Helvetica 14 bold')
        # ey_average_label.grid(row=stats_row, sticky=E, column=stats_column)
        # ey_average_entry = Label(self.frame_stats, textvariable=self.ey_average_var, width=stats_entry_width,)
        # ey_average_entry.grid(row=stats_row, column=stats_column+1)
        # unit_label = Label(self.frame_stats, text="(%)",)
        # unit_label.grid(row=stats_row, column=stats_column+2, sticky=W)
        ...
        # ey_stddev_label = Label(self.frame_stats, text="Scatter in EY:",, font='Helvetica 14 bold')
        # ey_stddev_label.grid(row=stats_row, sticky=E, column=stats_column)
        # ey_stddev_entry = Label(self.frame_stats, textvariable=self.ey_stddev_var, width=stats_entry_width,)
        ...
        eff_label = Label(
            self.frame_stats, text="Efficiency:", font="Helvetica 14 bold",
        )
        eff_label.grid(row=stats_row, sticky=E, column=stats_column)
        eff_entry = Label(
            self.frame_stats, textvariable=self.eff_var, width=stats_entry_width,
        )
        ...
        self.q_var.set("None")
        eff_label = Label(self.frame_stats, text="Quality:", font="Helvetica 14 bold",)
        eff_label.grid(row=stats_row, sticky=E, column=stats_column)
        eff_entry = Label(
            self.frame_stats, textvariable=self.q_var, width=stats_entry_width,
        )
        eff_entry.grid(row=stats_row, column=stats_column + 1)
        unit_label = Label(self.frame_stats, text="",)
        unit_label.grid(row=stats_row, column=stats_column + 2, sticky=W)
    ...
    def select_reference_object_mouse(self, event):
        ...
        # Update the user interface status
        self.status_var.set(
            "Use your mouse to click on one side of the reference object... You will then need to click again to set up a measuring line..."
        )
    ...
    def reset_status(self):
        ...
        self.quick_var.set(0)
        self.histogram_type.set(hist_choices[default_histogram_choice])
    ...
    def psd_hist_from_data(
        ...
        serr2 = plt.errorbar(
            ...
            ecolor=lighter(hist_color_fm, 0.3),
            markeredgewidth=1.5,
            markeredgecolor="k",
            capsize=capsize,
            capthick=capthick,
    ...
    def erase_clusters(self, event):
        ...
        if self.nclusters is None:
            ...
            # Update the user interface status
            self.status_var.set(
                "Coffee Particles not Detected Yet... Use Launch Particle Detection Button..."
            )
        ...
        if self.display_type.get() != outlines_image_display_name:
            ...
            # Update the user interface status
            self.status_var.set(
                "Please select 'Display Type' = 'Cluster Outlines' to use the 'Erase Clusters' tool."
            )
        ...
        if self.erase_clusters_mode is False:
            ...
            # Update the user interface status
            self.status_var.set(
                "Entered 'Erase Clusters' mode. Click with the mouse to erase all clusters within the circle. Zoom in or out to set precision. Hit Escape or the 'Erase Clusters' button to end."
            )
    ...
    def create_histogram(self, event):
        ...
        if self.pixel_scale_var.get() == "None":
            ...
            # Update the user interface status
            self.status_var.set(
                "The Pixel Scale Has Not Been Defined Yet... Use the Left Mouse Button to Draw a Line on a Reference Object and Choose a Reference Length in mm..."
            )
    ...
    def save_histogram(self, event):
        ...
        if self.display_type.get() == histogram_image_display_name:
            ...
            ihist = np.where(np.array(hist_choices) == self.histogram_type.get())
            image_code = hist_codes[ihist[0][0]]
